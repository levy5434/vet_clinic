...
from .functions import next_weekday, what_week, time_plus, free_hours, scheduler, whole_week
...
def doctor(request, doctor_id):
    ...
    slots = Slot.objects.filter(doctor=doctor,date__range=[first,last]).order_by('date')
...
def my_appointments(request):
    ...
    context = {'appointments': appointments,'past_appointments':past_appointments,'upcoming_appointment':upcoming_appointment}
...
def appointment(request,appointment_id):
    ...
    delete = False
    if appointment.date > date.today() and request.user == client or request.user.groups.filter(name__in=['Admin']).exists():
        delete = True
    if request.user != client and not request.user.is_staff:
        raise PermissionDenied
    if appointment.animal:
        animal = appointment.animal
        appointments = Appointment.objects.filter(animal=animal).exclude(id=appointment_id)
        ...
        context = {'animal':animal,'actual_diseases': actual_diseases,
                    'client':client,'past_diseases':past_diseases,'appointments':appointments,
                    'appointment':appointment,'delete':delete,'reschedule':reschedule}
    else:
        context = {'client':client,'appointment':appointment,'delete':delete,'reschedule':reschedule}                
...
def make_appointment_d(request,service_id):
    if request.user.groups.filter(name__in=['Admin']).exists():
        ...
        services = Service.objects.filter(admin_only=False).exclude(id=service_id)   
        my_service = Service.objects.get(id=service_id)
...
def make_appointment_s(request,service_id,doctor_id,week):
    ...
    all_slots = Slot.objects.filter(doctor=selected_doctor,date__range=[first,last]).order_by('date')
    ...
    context = {'services':services,'my_service':my_service,
                'doctors':doctors,'selected_doctor':selected_doctor, 'days':days, 'more_hours':more_hours,
...
def make_appointment_f(request,service_id,slot_id,time):
    ...
    if service.admin_only==True and not request.user.groups.filter(name__in=['Admin']).exists():
        messages.success(request, _('This service is not available!'))
        return redirect('app:make_appointment')
    ...
    if request.method!='POST':
        ...
        if form.is_valid():
            if service.admin_only==True:
                ...
                appointment.save()
                messages.success(request, _('Success! Free time has been booked.'))
                ...
                appointment.save()            
                messages.success(request, _('Success! Appointment has been booked.'))
                return redirect('app:slot',slot_id=slot.id)
...
def delete_appointment(request,appointment_id):
    ...
    if appointment.date < date.today():
        messages.success(request, _('You can not delete previous appointments'))
...
def add_client_appointment_f(request,appointment_id,client_id):
    ...
    if request.method=='POST':
        ...
        appointment.save()
        messages.success(request,_("Client has been added to the appointment"))
...
def client(request,client_id):
    client = User.objects.get(id=client_id)
    appointments = Appointment.objects.filter(client=client).order_by('date','start_time')
    past_appointments = []
    today = date.today()
    for appointment in appointments:
        if appointment.date < today:
    ...
    context = {'client':client,'animals': animals, 'appointments':appointments,
...
def add_disease(request,pet_id):
    ...
    if request.method!='POST':
        ...
        if form.is_valid():
            ...
            new_disease.doctor = f"Added by: {request.user.first_name} {request.user.last_name}"
            new_disease.save()
...
def slot(request, slot_id):
    ...
    context = {'slot': slot,'appointments':appointments,'upcoming_appointment':upcoming_appointment,'times2':times,'times1':times1}
...
def add_medicine(request,disease_id):
    ...
    if request.method!='POST':
        ...
        if form.is_valid():
            ...
            new_medicine.doctor = f"Added by: {request.user.first_name} {request.user.last_name}"
            new_medicine.save()
    ...
    if request.method != 'POST':
        ...
        if slot_form.is_valid() and day_form.is_valid():
            ...
            for d_date in dates:
                ...
                if ifslot:
                    messages.success(request, _('You can not add slot in this day of the week!'))
                    return redirect('app:add_slot',doctor_id=doctor_id)      
            for d_date in dates:
...
def reschedule(request,appointment_id,week, doctor_id):
    ...
    first = what_week(week)[0]
    last = what_week(week)[1]
    doctors = Doctor.objects.filter(services__id=service.id).exclude(id=doctor_id)
    context = {'appointment':appointment,'service':service,'doctor':doctor,
                'slots':slots, 'client':client,'week':week,'n_week':n_week,'p_week':p_week,
                'first':first,'last':last,'doctors':doctors}
...
def reschedule_confirm(request,appointment_id,slot_id,time):
    ...
    if not doctor.services.filter(id=service.id).exists():
        messages.success(request, _('This service is not available!'))
        return redirect('app:reschedule', service_id=service.id,
                                            doctor_id=appointment.doctor.id,week=0)
    available = False
    for f_time in times:
        if time == f_time.strftime("%H:%M:%S"):
            available = True
    ...
    if available == False:
        ...
        return redirect('app:reschedule', service_id=service.id,doctor_id=appointment.doctor.id,week=0)
    if request.method == "POST":
        ...
        return redirect('app:appointment',appointment_id=appointment.id)
    context = {'appointment':appointment,'slot':slot,'doctor':doctor,'client':client,'time':time}    
...
def edit_appointment(request,appointment_id):
    ...
    if request.method!='POST':
        if appointment.animal:
            ...
            if appointment.client:
                form.fields['animal'] = forms.ModelChoiceField(Animal.objects.filter(owner=appointment.client)
