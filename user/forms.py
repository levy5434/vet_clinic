...
from .models import Doctor, Profile, Slot
...
class ProfileForm(forms.ModelForm):
    city = forms.CharField(
        ...
        validators=[
            RegexValidator(
                regex="^([A-ZŁŚĆĄŻŹÓĆŃĘ]{1}[a-zążźśęćńół]{1,20}(\-|\ ){0,1}){1,4}$",
                message=_lazy(
                    "City should have first letter upper case and cannot contain special characters"
                ),
            )
        ],
        widget=forms.Textarea(
